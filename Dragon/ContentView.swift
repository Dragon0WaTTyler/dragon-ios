                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        case .empty:
                            BooksStateCard(
                                title: "No books yet",
                                message: "The backend responded successfully, but there are no books to show right now.",
                                buttonTitle: "Reload"
                            ) {
                                await viewModel.loadBooks()
                            }
...
struct DragonYouTubeView: View {
    @StateObject private var viewModel: DragonYouTubeSectionsViewModel

    init() {
        _viewModel = StateObject(wrappedValue: DragonYouTubeSectionsViewModel())
    }

    init(viewModel: DragonYouTubeSectionsViewModel) {
        _viewModel = StateObject(wrappedValue: viewModel)
    }
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        case .failed(let message):
                            DragonYouTubeSectionsStateCard(
                                title: "Could not load YouTube sections",
                                message: message,
                                buttonTitle: "Try Again"
                            ) {
                                await viewModel.loadSections()
                        ...
                        case .empty:
                            DragonYouTubeSectionsStateCard(
                                ...
                                message: "The backend responded successfully, but there are no YouTube sections to show right now.",
                                buttonTitle: "Reload"
                            ) {
                                await viewModel.loadSections()
                            }
                        ...
                        case .loaded, .loading:
                            LazyVStack(spacing: 12) {
                                ForEach(viewModel.sections) { section in
                                    NavigationLink {
                                        DragonYouTubeSectionDetailView(section: section)
                                    } label: {
                                        DragonYouTubeSectionRow(section: section)
                                    }
                                    .buttonStyle(.plain)
                                }
                            }
...
#Preview("YouTube Sections") {
    DragonYouTubeView(
        viewModel: DragonYouTubeSectionsViewModel(
            initialState: .loaded,
            initialResponse: .preview
        )
    )
...
struct DragonSettingsView: View {
    ...
    @State private var articleCacheCount = 0
    @State private var movieCacheCount = 0
    @State private var totalCacheCount = 0
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Cache and backend settings are device-local and can persist across app upgrades.")
                        .font(.caption)
                        .foregroundStyle(.gray)
    ...
    private var appBuildLabel: String {
        ...
        return "App: \(bundleID) • v\(version) (\(build))"
    }
    ...
    private func refreshCacheInfo() async {
        ...
        defer { isRefreshingCacheInfo = false }

        do {
            articleCacheCount = try await responseCache.cacheItemCount(for: .articles)
            movieCacheCount = try await responseCache.cacheItemCount(for: .movies)
            totalCacheCount = try await responseCache.cacheItemCount()
            cacheStatusText = nil
        } catch {
    ...
    private func clearAllCache() async {
        ...
        do {
            ...
            cacheStatusText = "All Dragon response cache cleared."
            articleCacheCount = 0
            movieCacheCount = 0
            totalCacheCount = 0
        } catch {
            cacheStatusText = "Could not clear cache."
        }
    }
}
